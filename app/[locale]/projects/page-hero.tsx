...
// ── Project Page Client ───────────────────────────────────────
export default function ProjectPageClient({ posts }: { posts: Post[] }) {
      ...
      <div className="relative max-w-5xl mx-auto px-6 pt-10 pb-20">
        ...
        {rest.length > 0 && (
          <div className={`flex items-center gap-4 mb-8 transition-all duration-700 delay-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            <span className="text-xs font-semibold uppercase tracking-widest text-black/30">{dict.projects.all_posts}</span>
        ...
        {posts.length === 0 && (
          <div className="text-center py-24 text-black/30">
            <p className="text-lg font-semibold">{dict.projects.no_projects}</p>
            <p className="text-sm mt-1">{dict.projects.check_back}</p>
          </div>
        )}
      </div>
