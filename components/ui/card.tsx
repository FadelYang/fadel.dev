    ...
    <Link href={localizedHref} className="group block">
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-stretch">
          {post.cover && (
            <div className="w-full lg:w-1/2 aspect-video rounded-xl overflow-hidden border border-black/5 relative shrink-0 bg-black/5">
              <img
                src={post.cover}
                alt={post.title}
...
export function PostCard({ post, index, visible }: { post: Post; index: number; visible: boolean }) {
  ...
  return (
    <Link
      ...
      >
        {post.cover && (
          <div className="w-full aspect-video rounded-xl overflow-hidden border border-black/5 mb-4 relative bg-black/5">
            <img
              src={post.cover}
              alt={post.title}
              className="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-500 ease-out"
